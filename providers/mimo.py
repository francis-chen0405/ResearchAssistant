...
def _direct_mimo_prompt(request: LLMRequest) -> str:
    ...
    if request.stage is LLMStage.EXTRACTOR:
        stage_compatibility = (
            ...
            "The selected_sentence_ranges array must never be empty. If one relevant sentence "
            "is too short, select a contiguous range of adjacent relevant sentences until the "
            "minimum is met; never return an empty array.\n"
            "Use at least 20 exact quoted words only when the quotation contains at least "
            ...
            "count. Otherwise, use at least 30 exact quoted words. A digit without a marker "
            "and a marker without a digit both require 30 words. Preserve material "
            "qualifications and do not use unrelated padding. Return exact source text; never "
            "paraphrase, heal, expand, or invent context. Never repair a short quote. "
            "Claim-keyword matches are audit metadata, not a hard acceptance gate. Python "
        ...
        stage_compatibility = (
            ...
            "If an opposing candidate instead supports the claim, assign claim_fit at most 1 "
            ...
            "claim_fit >= 2. The application derives approved, ledger_score, and placement from "
            "the semantic scores. Claim Fit 2 is qualified-only; Claim Fit 3 is ordinary eligible "
            "evidence.\n"
            "</DIRECT_MIMO_ANALYST_STANCE_COMPATIBILITY>\n"
        )
