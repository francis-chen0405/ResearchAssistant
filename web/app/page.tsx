...
function ProviderSetup({ configuration, selectedProviders, onClose, onSaved }: { configuration: Configuration | null; selectedProviders: ProviderSelection; onClose: () => void; onSaved: (message: string) => Promise<void> }) {
  ...
  const remove = async (name: string) => {
    ...
    try {
      await researchApi.removeCredential(name);
      setMimo(""); setLuna(""); setSerpsearch(""); setExa(""); setOpenalex(""); setPubmed(""); setFirecrawl("");
      await onSaved("Saved credential removed.");
    } catch { setError("The credential could not be removed. Wait for active research to finish and check the credential vault."); }
    finally { setSaving(false); }
  };
  const credentialNames: Record<string, string> = { mimo: "MIMO_API_KEY", openai: "LUNA_API_KEY", serpsearch: "SERPSEARCH_API_KEY", exa: "EXA_API_KEY", openalex: "OPENALEX_API_KEY", pubmed: "PUBMED_API_KEY", firecrawl: "FIRECRAWL_API_KEY" };
