...
export default function Home() {
  ...
  const [settings, setSettings] = useState<Settings>({ dbPath: "", runId: "", maxTokens: 200_000, maxCost: "0.15", maxCalls: 160, supportEnabled: true, challengeEnabled: false, sourceTarget: 10, useSerpSearch: true, useExa: true, useOpenAlex: true, useArxiv: false, usePubmed: false, useCrossref: true });
    ...
    <MotionConfig reducedMotion="user" transition={{ duration: 0.42, ease: [0.22, 1, 0.36, 1] }}>
      <main className="site-shell">
        ...
        <AnimatePresence>
          ...
          {setupOpen && <ProviderSetup key="provider-setup" configuration={configuration} onClose={() => setSetupOpen(false)} onSaved={async (message) => { setNotice(message); await refreshConfiguration(); }} />}
...
function Header({ view, providerState, onView, onSetup, onAdvanced }: { view: MainView; providerState: "offline" | "ready" | "saved" | "setup"; onView: (view: MainView) => void; onSetup: () => void; onAdvanced: () => void; }) {
  return <header className="topbar">
    ...
    <div className="header-actions"><button className="advanced-link" type="button" onClick={onAdvanced}>Advanced</button><button className="setup-link" type="button" onClick={onSetup}><span className={`status-dot ${providerState}`} aria-hidden="true" />{providerState === "ready" ? "Providers ready" : providerState === "saved" ? "Provider keys saved" : providerState === "offline" ? "Local API offline" : "Provider setup"}</button></div>
  </header>;
}
