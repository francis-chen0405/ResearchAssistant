...
function ProviderSetup({ onClose, onSaved }: { onClose: () => void; onSaved: (message: string) => void }) {
  const [mimo, setMimo] = useState(""); const [luna, setLuna] = useState(""); const [serpsearch, setSerpsearch] = useState(""); const [exa, setExa] = useState(""); const [openalex, setOpenalex] = useState(""); const [pubmed, setPubmed] = useState(""); const [firecrawl, setFirecrawl] = useState(""); const [saving, setSaving] = useState(false); const [error, setError] = useState<string | null>(null);
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setSaving(true); setError(null); try { const result = await researchApi.saveCredentials({ ...(mimo ? { mimo_api_key: mimo } : {}), ...(luna ? { luna_api_key: luna } : {}), ...(serpsearch ? { serpsearch_api_key: serpsearch } : {}), ...(exa ? { exa_api_key: exa } : {}), ...(openalex ? { openalex_api_key: openalex } : {}), ...(pubmed ? { pubmed_api_key: pubmed } : {}), ...(firecrawl ? { firecrawl_api_key: firecrawl } : {}) }); setMimo(""); setLuna(""); setSerpsearch(""); setExa(""); setOpenalex(""); setPubmed(""); setFirecrawl(""); onSaved(result.message); } catch (caught) { setError(caught instanceof Error ? caught.message : "The keys could not be saved."); } finally { setSaving(false); } };
  return <Modal title="Connect the research providers" onClose={onClose}><p className="modal-intro">Keys go directly to your macOS Keychain through the local API. They are never returned to this page. arXiv and Crossref do not require keys; Crossref supplies metadata only, never evidence.</p><form className="setup-form" onSubmit={submit}><label>MiMo API key <span>leave blank to keep the saved key</span><input type="password" value={mimo} onChange={(event) => setMimo(event.target.value)} autoComplete="off" /></label><label>OpenAI API key <span>for GPT-5.6 Luna gap analysis and evidence analysis</span><input type="password" value={luna} onChange={(event) => setLuna(event.target.value)} autoComplete="off" /></label><label>SERP Search API key <span>optional until selected</span><input type="password" value={serpsearch} onChange={(event) => setSerpsearch(event.target.value)} autoComplete="off" /></label><label>Exa API key <span>optional until selected</span><input type="password" value={exa} onChange={(event) => setExa(event.target.value)} autoComplete="off" /></label><label>OpenAlex API key <span>optional until selected</span><input type="password" value={openalex} onChange={(event) => setOpenalex(event.target.value)} autoComplete="off" /></label><label>PubMed API key <span>optional; higher request allowance when PubMed is selected</span><input type="password" value={pubmed} onChange={(event) => setPubmed(event.target.value)} autoComplete="off" /></label><label>Firecrawl API key <span>optional fallback</span><input type="password" value={firecrawl} onChange={(event) => setFirecrawl(event.target.value)} autoComplete="off" /></label>{error && <p className="form-error">{error}</p>}<button className="primary-action wide" type="submit" disabled={!(mimo || luna || serpsearch || exa || openalex || pubmed || firecrawl) || saving}>{saving ? "Saving securely…" : "Save to Keychain"}<span>↗</span></button></form></Modal>;
}
