...
Status: Authorized by the user on 2026-08-30; hosted boundary and canonical worker adapter
verified locally, staging verification pending.

## Objective

Move the live product boundary from local SQLite/Keychain operation to a hosted
...
- Render hosts the public Next.js web service, private FastAPI API service, and
  persistent background worker. Render Cron, Postgres, Key Value, and Workflows are
  not part of v1.
...
- Hosted browser calls are same-origin Next.js server-proxy calls. The private API,
  service-role key, database credentials, and provider secrets never reach browser
  state, URLs, logs, artifacts, or build output.
...
- `frontend/hosted_api.py`, `render_api.py`, `hosted_worker.py`, and `hosted_canonical.py`
  for the private authenticated API and persistent worker boundary.
...
execution, Render private networking, and real worker/provider smoke tests require configured
external accounts; they must pass before any production cutover. The worker loads its
